...
type TFormValues = FormattedCoupon;

export default function CouponDataTable({ result, locale }: { result: ActionResult<TFormValues>; locale: TLocalesData }) {
	...
	const handleDeleteMany = async (ids: string[]) => {
		...
		const result = await deleteManyCouponsAction(ids);
		setResponse(result);
	};
	...
	const columns: ColumnDef<TFormValues>[] = [
		...
		{ accessorKey: 'type', header: 'columns.coupon_type' },
		{
			accessorKey: 'value',
			header: 'columns.coupon_value',
			cell: ({ row }) => (
