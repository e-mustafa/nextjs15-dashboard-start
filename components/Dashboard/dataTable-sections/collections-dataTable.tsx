...
export default function CollectionDataTable({ result, locale }: { result: ActionResult<Collection>; locale: TLocalesData }) {
	...
	const handleServerRequest = useCallback(
		async ({ page, limit, search, sortBy, sortOrder }: TQueryParams) => {
			...
			startTransition(() => {
				setOptimisticData((res.data as Collection[]) ?? []);
	...
	const handleDelete = async (id: string) => {
		...
		// await runWithFeedback(() => deleteCollectionAction(id));
	...
	const handleDeleteMany = async (ids: string[]) => {
		...
		const result = await deleteManyCollectionsAction(ids);
		setResponse(result);
	};
