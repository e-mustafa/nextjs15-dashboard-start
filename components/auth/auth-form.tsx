...
interface Props<T extends FieldValues> {
	schema: (
		t: Tt
	) => ZodObject<T extends object ? T : Record<string, unknown>> | ZodType<T extends object ? T : Record<string, unknown>>;
	defaultValues: T;
...
export default function AuthForm<T extends FieldValues>({ type, schema, defaultValues, onSubmit }: Props<T>) {
	...
	const form = useForm({
		resolver: zodResolver(schema(t as Tt) as ZodType<T extends object ? T : Record<string, unknown>, FieldValues>),
