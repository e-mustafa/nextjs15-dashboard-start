...
import { createCoupon, deleteManyCoupons, getAllCoupons } from '@/server/services/coupon-service';

// ✅ Route GET Handler
async function getCouponsAction(req: Request) {
	...
	return getAllCoupons(params, acceptLanguage as TLocalesData);
}

// ✅ Route POST Handler
async function createCouponAction(req: Request) {
...
async function deleteCouponsAction(req: Request) {
	const { ids } = await req.json();
	return deleteManyCoupons(ids);
}

// ✅ export with centralized error handling
export const { GET, POST, DELETE } = errorHandler({
	GET: getCouponsAction,
	POST: createCouponAction,
	DELETE: deleteCouponsAction,
});
